...
public interface AuditLogRepository extends JpaRepository<AuditLog, Long> {
    ...
    @Query("SELECT COUNT(DISTINCT a.username) FROM AuditLog a WHERE a.action = :action AND a.createdAt >= :date")
    long countDistinctUsersByActionAndCreatedAtAfter(@Param("action") String action, @Param("date") LocalDateTime date);
}
